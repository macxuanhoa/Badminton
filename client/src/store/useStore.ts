...
interface AppState {
  ...
  createOrder: (payload: Omit<OrderRecord, 'id' | 'createdAt' | 'status'>) => Promise<string>;
...
export const useStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      products: [
        { id: 'p1', name: 'Nước suối Aquafina', price: 15000, stock: 50, minStock: 10, category: 'Nước uống' },
        { id: 'p2', name: 'Nước tăng lực Redbull', price: 25000, stock: 30, minStock: 5, category: 'Nước uống' },
        { id: 'p3', name: 'Bóng Pickleball Franklin', price: 85000, stock: 20, minStock: 2, category: 'Dụng cụ' },
        { id: 'p4', name: 'Quấn cán vợt Yonex', price: 45000, stock: 100, minStock: 20, category: 'Dụng cụ' }
      ],
      user: null,
      isLoading: false,
      error: null,
      ...
      fetchInitialData: async () => {
        ...
        try {
           ...
           const [bookingsRes, productsRes] = await Promise.all([
             hasToken ? api.get('/bookings').catch(() => ({ data: [] })) : Promise.resolve({ data: [] }),
             api.get('/products').catch(() => ({ data: [] }))
           ])
           set({ bookings: bookingsRes.data, products: productsRes.data })
        } finally {
          set({ isLoading: false })
        }
      },
      ...
      createOrder: async (payload) => {
        const id = uid()
        const record: OrderRecord = { ...payload, id, createdAt: Date.now(), status: 'NEW' }
        ...
        set((state) => {
          ...
          return { 
            ...
            user: state.user ? { 
              ...state.user, 
              walletBalance: state.user.walletBalance - payload.total,
              points: state.user.points + Math.floor(payload.total / 1000)
            } : null
          };
        })
        return id
